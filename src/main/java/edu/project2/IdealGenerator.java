...
public class IdealGenerator implements Generator {
    ...
    private void buildUpTheMazeByAddingRandomWalls(Cell[][] grid, int height, int width) {
        for (int k = 0; k < height * width * wallGenerationAttemptCoefficient; k++) {
            ...
            if (checkThatOnlyOneNeighborCellIsWall(grid, row, col)) {
    ...
    private void addExtraWallsIfPossible(Cell[][] grid, int height, int width) {
        for (int i = 1; i < height - 1; i++) {
            for (int j = 1; j < width - 1; j++) {
                if (checkThatOnlyOneNeighborCellIsWall(grid, i, j)) {
    ...
    private boolean checkThatOnlyOneNeighborCellIsWall(Cell[][] grid, int row, int col) {
